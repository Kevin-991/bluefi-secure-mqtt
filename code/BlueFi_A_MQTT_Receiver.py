...
print("===== BlueFi A Encrypted MQTT Receiver =====")
...
print("A board IP:", wifi.esp.ipv4_address)
...
def connected(client, userdata, flags, rc):
    ...
    print("Subscribing:", TOPIC_B_TO_A)
    client.subscribe(TOPIC_B_TO_A)
...
def message(client, topic, msg):
    ...
    try:
        ...
        if cmd == "HELLO":
            reply = build_frame(seq, "ACK", "ACK_FROM_A")
            print("Publishing encrypted ACK")
            print("ACK frame:", reply)
            client.publish(TOPIC_A_TO_B, reply)
        else:
            reply = build_frame(seq, "ERR", "UNKNOWN_CMD")
            print("Publishing encrypted ERR")
            client.publish(TOPIC_A_TO_B, reply)

    except Exception as e:
        print("Frame parse error:", e)
        error_frame = build_frame(0, "ERR", "BAD_FRAME")
        client.publish(TOPIC_A_TO_B, error_frame)
...
print("A board is waiting for encrypted message...")
